...
const rows = [
  { id: 'year', numeric: false, disablePadding: true, label: 'Semestre' },
...
class StudentTable extends React.Component {
  ...
  //when all the students are selected by once
  handleSelectAllClick = (event, data) => {
    if (event.target.checked) {
      this.setState(state => ({ selected: state.data.map(n => n.id) }));
      this.props.studentsSelected(data.map(item => {
        item.visibility='1';
        return item;
      }));
      return;
  ...
  //when the students are being selected one by one
  handleClick = (event, id, student) => {
    ...
    if(selectedIndex <= -1)
      student.visibility='1';
    else
      student.visibility='0';
